...
def nms_pytorch(boxes, scores, iou_thres):
    """
    纯 PyTorch 实现的 NMS,替代 torchvision.ops.nms。
    避免因 torch/torchvision 版本不匹配导致的
    "Couldn't load custom C++ ops" 错误。
    ...
    while order.numel() > 0:
        ...
        # 保留 IoU 低于阈值的框
        idx = (iou <= iou_thres).nonzero(as_tuple=False).squeeze(1)
        order = order[idx + 1]
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = nms_pytorch(boxes, scores, iou_thres)  # NMS (纯PyTorch,避免torchvision C++ ops问题)
